...
**Created:** 2026-02-28
**Base commit:** (use current HEAD — SQLite parser handoff should be complete)
**Branch:** main
**Mode:** Standard
...
## Objective
Implement a DDL parser that extracts SemanticSnapshot from raw `CREATE TABLE` SQL files. This is a Tier 1 parser — universal format, zero new dependencies (`sqlparse` is already installed). While building it, clean up two minor interface issues: remove the dead `snapshot_name` parameter from the parse path, and refactor `detect_source_type()` to delegate to the registry for file-path detection.

## Current State
The parser registry already has 4 parsers: `dbt_manifest`, `dbt_project`, `lookml`, `sqlite`. 72 tests passing. This handoff adds `ddl` as the 5th parser.
...
### Why clean up now
The DDL parser is the first parser built from scratch on the plugin interface (the SQLite parser was also new, but it didn't need detect_source_type or parse_from_path changes). Cleaning the dead parameter and duplicated detection logic now means every subsequent parser (Django, SQLAlchemy, Prisma, etc.) builds on a tight foundation.
...
**Register DDLParser in `get_default_registry()`** — add after the existing SQLiteParser registration:
```python
...
Also update the existing `test_parse_from_path_unsupported_raises` if needed — use `.xyz` extension to avoid any parser matching:
```python
...
2. `python -m pytest datashark-protocol/tests/test_parsers.py -v` — expected: all 48+ parser tests pass (72 baseline minus non-parser tests, plus ~25 new DDL tests)
3. `python -m pytest datashark-protocol/tests/ -v` — expected: ~95+ total tests pass (72 baseline + ~25 new)
...
- Implement other parsers (Django, SQLAlchemy, etc.) — DDL only in this handoff
- Add async to any parser code
- Change the hashing logic in `build_snapshot()`
- Modify existing dbt, LookML, or SQLite parser logic (only registry and __init__ change)
