...
# Note: log_sql_generation removed - artifact logging is handled at engine level
...
class SQLBuilder:
    ...
    def build_final_sql(
        ...
        **Architectural Contract:** The SemanticSnapshot is the sole source of truth for
        ...
        # to ensure all entrypoints are covered. This builder does not log artifacts.
            ...
            if date_trunc_field and field_id == date_trunc_field and date_trunc_unit:
                ...
                if field_def:
                    ...
                    if entity:
                        ...
                        # Fallback: no table qualification
                        date_trunc_expr = self.dialect.render_date_trunc(
                    ...
                    # Fallback: use field_name directly
                    date_trunc_expr = self.dialect.render_date_trunc(
                        self.dialect.quote_identifier(field_name),
                        date_trunc_unit
                    )
    ...
    def _build_join_clauses(self, join_path: List) -> List[str]:
        ...
        for join_item in join_path:
            ...
            if isinstance(join_item, dict):
                ...
                # Extract field names from field_ids
    ...
    def _escape_value(self, value: Any) -> str:
        ...
        if isinstance(value, str):
            ...
            # Non-string values are safe to convert directly
            return str(value)
    ...
    def _render_filter_def(self, filter_def: FilterDef, snapshot: Optional[SemanticSnapshot] = None) -> str:
        ...
        """
        field_name = filter_def.field_id.split(":")[-1]  # Extract field name from field_id
        
        # Get fully qualified field reference (entity.field)
        if snapshot and filter_def.entity_id:
            # Get entity name from snapshot
            ...
            if entity:
                ...
                # Fallback: use field name only if entity not found
                field_ref = self.dialect.quote_identifier(field_name)
        elif snapshot:
            # Try to find entity from field_id
            ...
            # No snapshot available, use field name only (legacy behavior)
            field_ref = self.dialect.quote_identifier(field_name)
        ...
        if aggregation_required and grouping_fields:
            ...
            for field_name in grouping_fields:
                ...
                if date_trunc_field and date_trunc_unit:
                    # Find the field_id for this field_name
                    field_id = None
                    ...
                    if field_id == date_trunc_field:
                        # Use DATE_TRUNC expression in GROUP BY
                        field_def = snapshot.fields.get(field_id)
                        if field_def:
                            entity = snapshot.entities.get(field_def.entity_id)
                            if entity:
